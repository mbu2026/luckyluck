...
function updateFrequencyWithLottoData() {
    ...
    dataToProcess.forEach(draw => {
        console.log('draws parsing'+draw.drawSystemId);
        if (draw.resultsJson) {
            draw.resultsJson.forEach(number => {
                if (numberFrequency.hasOwnProperty(number)) {
...
function updateAnalytics() {
    ...
    for (let i = min; i <= max; i++) {
        ...
        if (freq < minFreq && freq > 0) {
            minFreq = freq;
            leastFrequent = [i];
            console.log('leastFrequent'+leastFrequent);
    ...
    for (let i = min; i <= max; i++) {
        if (numberFrequency[i] > 0) {
            freqPairs.push({number: i, frequency: numberFrequency[i]});
        }
    }
